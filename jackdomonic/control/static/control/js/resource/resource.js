...
function sospensioneRisorse(resource,plc,index){

    $('#dg').datagrid('selectRow',index);
    var prenotazione = $('#dg').datagrid('getSelected');
    $.ajax({
		url: sospensione,
		type: 'POST',
			...
			try {
				...
				switch (status) {
					case 200:
						...
						$.messager.alert('Sistema', 'Funzioni Risorsa  ' + resource + ' sospesa ', 'info');
...
$(document).ready(function() {
		...
		columns: [[
			...
			{ field: 'schedulerId', title: "Scheduling Ref.", align: 'center', hidden: false },
			{
				...
				formatter: function(value, row, index) {
				    ...
				    var  formatted_tex = "<div></div>";
				    var status = parseInt(value);
				    switch(row.status){
				    case 0:
				    case 6:
				    formatted_tex =  "<button  onclick = forzaAvvioRisorsa(\"" + row.tag + "\",\"" + row.plcRef + "\")  class=\"easyui-linkbutton\" data-options=\"iconCls:'icon-reload'\" style=\"width:80px'\" value=\"Forza Avvio\">Avvia</button>";
				    break;
				    case 1:
				    //sospensioneRisorse
				    formatted_tex =  "<button  onclick = sospensioneRisorse(\"" + row.tag + "\",\"" + row.plcRef + "\",\"" + index+ "\")  class=\"easyui-linkbutton\" data-options=\"iconCls:'icon-reload'\" style=\"width:80px'\" value=\"Forza Avvio\">Sospendi</button>";
				    break;
				    }
	...
	pager.pagination({
		...
		onSelectPage: function(pageNumber, pageSize) {
			//setQueryStringOrdini("prentazioni", pager);
	...
	$('#modPrenotazione').window({
		href:window.location.origin+'/control/gestione-prenotazione',
		width:500,
    	height:400,
